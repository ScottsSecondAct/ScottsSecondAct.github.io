...
flowchart LR
    ...
    DNA -->|"mRNA exported\nthrough nuclear pore"| RIB["Ribosomes\non RER"]

    subgraph ER["📦 Rough ER"]
        RIB -->|"protein synthesis\n& folding"| FOLD["Protein\nfolding +\nglycosylation"]
    end

    FOLD -->|"transport\nvesicle buds off"| CIS["cis face\n(receiving)"]

    subgraph GOLGI["🏭 Golgi Apparatus"]
        CIS -->|"further\nmodification"| MED["Medial\ncisternae"]
        MED -->|"sorting &\ntagging"| TRANS["trans face\n(shipping)"]
    end

    TRANS -->|"secretory\nvesicle"| PM["🔲 Plasma Membrane\n(exocytosis)"]
    TRANS -->|"transport\nvesicle"| LYS["♻️ Lysosome\n(intracellular digestion)"]
    TRANS -->|"transport\nvesicle"| STAY["📍 Remains\nin cell"]

    style NUCLEUS fill:#e8f0fe,stroke:#4285f4
    style ER fill:#fef7e0,stroke:#f9ab00
    style GOLGI fill:#e6f4ea,stroke:#34a853
    style PM fill:#fce8e6,stroke:#ea4335
    style LYS fill:#f3e8fd,stroke:#a142f4
```
...
```mermaid
graph TD
    CS["🧬 Cytoskeleton"] --> MF["Microfilaments\n(Actin)\n~7 nm"]
    CS --> IF["Intermediate\nFilaments\n~8–12 nm"]
    CS --> MT["Microtubules\n(Tubulin)\n~25 nm"]

    MF --> MF1["Cell shape & movement"]
    MF --> MF2["Muscle contraction\n(actin + myosin)"]
    MF --> MF3["Cleavage furrow"]

    IF --> IF1["Structural support\n& tension"]
    IF --> IF2["Anchor nucleus\n& organelles"]
    IF --> IF3["Keratin in\nskin/hair/nails"]

    MT --> MT1["Vesicle transport\ntracks"]
    MT --> MT2["Mitotic spindle"]
    MT --> MT3["Core of cilia\n& flagella (9+2)"]

    style CS fill:#f0f0f0,stroke:#333
    style MF fill:#ffe0e0,stroke:#c00
