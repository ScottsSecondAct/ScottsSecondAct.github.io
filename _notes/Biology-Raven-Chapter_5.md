...
```mermaid
graph TD
    A["🔬 Membrane Transport"] --> B["Passive Transport<br/>(No ATP)"]
    A --> C["Active Transport<br/>(Requires ATP)"]
    
    B --> D["Simple Diffusion<br/>Small, nonpolar molecules<br/>O₂, CO₂, steroids"]
    B --> E["Facilitated Diffusion<br/>Channel / carrier proteins<br/>Ions, glucose, amino acids"]
    B --> F["Osmosis<br/>Water only<br/>via aquaporins or bilayer"]
    
    C --> G["Primary Active<br/>Direct ATP hydrolysis"]
    C --> H["Secondary Active<br/>Ion gradient-coupled"]
    C --> I["Bulk Transport<br/>Vesicle-mediated"]
    
    G --> G1["Na⁺/K⁺ ATPase<br/>3 Na⁺ out, 2 K⁺ in"]
    G --> G2["Ca²⁺ ATPase"]
    G --> G3["H⁺/K⁺ ATPase"]
    G --> G4["ABC Transporters<br/>(CFTR, TAP, etc.)"]
    
    H --> H1["Symport<br/>Same direction<br/>Na⁺/glucose"]
    H --> H2["Antiport<br/>Opposite direction<br/>Na⁺/Ca²⁺"]
    
    I --> I1["Phagocytosis<br/>Large particles<br/>Clathrin-coated"]
    I --> I2["Pinocytosis<br/>Fluids / solutes"]
    I --> I3["Potocytosis<br/>Caveolin-coated<br/>Transcytosis"]
    I --> I4["Receptor-Mediated<br/>Specific ligands<br/>Clathrin-coated"]
    I --> I5["Exocytosis<br/>Secretion outward"]
```
...
```mermaid
graph TD
    PM["Plasma Membrane<br/>5–10 nm thick"] --> L["Lipids (~40%)"]
    PM --> P["Proteins (~50%)"]
    PM --> CHO["Carbohydrates (~10%)"]
    
    L --> PL["Phospholipids<br/>Bilayer foundation<br/>Amphiphilic"]
    L --> CHOL["Cholesterol<br/>Fluidity buffer<br/>Lipid rafts"]
    
    P --> IP["Integral Proteins<br/>Embedded in bilayer<br/>Removed by detergent"]
    P --> PP["Peripheral Proteins<br/>Surface-attached<br/>Removed by salt wash"]
    
    IP --> TM["Transmembrane<br/>Span full bilayer<br/>20–25 aa α-helix"]
    IP --> MONO["Monotopic<br/>One leaflet only"]
    
    IP --> FUNC_I["Functions:<br/>Channels, carriers,<br/>receptors, adhesion"]
    PP --> FUNC_P["Functions:<br/>Enzymes, signaling,<br/>cytoskeletal anchors"]
    
    CHO --> GP["Glycoproteins<br/>(on proteins)"]
    CHO --> GL["Glycolipids<br/>(on lipids)"]
    GP --> GC["Glycocalyx<br/>Cell identity<br/>Self vs. non-self"]
    GL --> GC
```
...
graph LR
    subgraph "Hypotonic"
        HY1["ECF solute < cytoplasm"] --> HY2["H₂O enters cell"]
        HY2 --> HY3A["Animal: Lyses 💥"]
        HY2 --> HY3B["Plant: Turgid ✅"]
    end
    
    subgraph "Isotonic"
        IS1["ECF solute = cytoplasm"] --> IS2["No net H₂O flow"]
        IS2 --> IS3A["Animal: Normal"]
        IS2 --> IS3B["Plant: Flaccid"]
    end
    
    subgraph "Hypertonic"
        HT1["ECF solute > cytoplasm"] --> HT2["H₂O leaves cell"]
        HT2 --> HT3A["Animal: Crenates 🫠"]
        HT2 --> HT3B["Plant: Plasmolysis"]
    end
```
