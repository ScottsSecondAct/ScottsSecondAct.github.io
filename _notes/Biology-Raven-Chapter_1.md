...
```mermaid
graph TD
    Start[Observation/Problem] --> Question[Ask Question]
    Question --> Hypothesis[Form Hypothesis<br/>testable & falsifiable]
    Hypothesis --> Prediction[Make Prediction<br/>If...then format]
    Prediction --> Experiment[Design & Conduct<br/>Controlled Experiment]
    Experiment --> Data[Collect & Analyze Data]
    Data --> Match{Results Match<br/>Prediction?}
    
    Match -->|Yes| Support[Hypothesis Supported<br/>not proven]
    Match -->|No| Reject[Hypothesis Not Supported]
    
    Support --> MoreTests[More Testing<br/>& Replication]
    MoreTests --> StillSupported{Consistently<br/>Supported?}
    StillSupported -->|Yes| Theory[Scientific Theory<br/>High Confidence]
    StillSupported -->|No| Modify
    
    Reject --> Modify[Modify or Generate<br/>New Hypothesis]
    Modify --> Hypothesis
    
    Theory --> OngoingResearch[Ongoing Research<br/>& Refinement]
    
    style Theory fill:#99ff99
    style Reject fill:#ff9999
    style Support fill:#ffff99
    style Experiment fill:#99ccff
```
...
```mermaid
graph TB
    Root[Common Ancestor<br/>~3.5-4 BYA] --> Split{Evolutionary<br/>Divergence}
    
    Split --> Bacteria[DOMAIN: BACTERIA<br/>Prokaryotes]
    Split --> ArchBranch[Archaea/Eukarya<br/>Common Ancestor]
    
    ArchBranch --> Archaea[DOMAIN: ARCHAEA<br/>Prokaryotes]
    ArchBranch --> Eukarya[DOMAIN: EUKARYA<br/>Eukaryotes]
    
    Bacteria --> B1[Spirochetes<br/>Proteobacteria<br/>Cyanobacteria]
    
    Archaea --> A1[Methanosarcina<br/>Halophiles<br/>Thermoplasma<br/>Extremophiles]
    ...
    style Bacteria fill:#ff9999
    style Archaea fill:#ffcc99
