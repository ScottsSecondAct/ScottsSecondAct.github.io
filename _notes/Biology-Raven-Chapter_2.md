...
```mermaid
graph TD
    A[Water H₂O Molecule] --> B[Polar Structure]
    B --> C[δ- on Oxygen]
    B --> D[δ+ on Hydrogens]
    C --> E[Hydrogen Bonding]
    D --> E
    E --> F[High Specific Heat]
    E --> G[High Heat of Vaporization]
    E --> H[Cohesion & Surface Tension]
    E --> I[Adhesion & Capillary Action]
    E --> J[Excellent Solvent]
    E --> K[Ice Less Dense Than Liquid]
    
    F --> L[Temperature Regulation]
    G --> M[Evaporative Cooling]
    H --> N[Plant Water Transport]
    I --> N
    J --> O[Biochemical Reactions]
    K --> P[Aquatic Life Protection]
...
```mermaid
graph LR
    A[Two Atoms Interact] --> B{ΔElectronegativity?}
    B -->|Large| C[Ionic Bond]
    B -->|Moderate| D[Polar Covalent Bond]
    B -->|Small/None| E[Nonpolar Covalent Bond]
    C --> F[Electron Transfer → Ions]
    D --> G[Unequal Sharing → δ+ / δ-]
    E --> H[Equal Sharing → No Partial Charges]
    F --> I[Crystal Lattice e.g. NaCl]
    G --> J[Polar Molecule e.g. H₂O]
    H --> K[Nonpolar Molecule e.g. O₂, CH₄]
...
```mermaid
graph LR
    A[pH 0-2: Stomach Acid] --> B[pH 3-6: Acidic]
    B --> C[pH 7: Neutral Water]
    C --> D[pH 7.4: Blood]
    D --> E[pH 8-10: Basic]
    E --> F[pH 11-14: Strong Base]
    
    G[More H⁺ ions] --> A
    H[Fewer H⁺ ions] --> F
    
    C --> I[Most cells: pH 6.8-7.4]
    I --> J[Enzyme Activity]
    J --> K[Buffer Systems Required]
